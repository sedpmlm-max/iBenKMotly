...
- **Curse** players (range: same region + adjacent 1 region):
  - On curse: victim EP → 0 (cannot move, attack, or rest)
  - A whisper arrives with a question — answer it to lift the curse
  - On correct answer: EP → maxEp (full recovery)
  - On timeout or bot detection: EP stays 0
  - On server error: EP → maxEp (forced release, not victim's fault)
  - **Curse resolution is top priority** — EP 0 blocks all cooldown actions
