...
`view.connectedRegions` contains adjacent region entries, and each entry is **either**
a full `Region` object (when the adjacent region is within vision) or a bare string ID
(when it is outside vision). Always type-check before use.

```
if typeof region === 'string' → it is an ID
if typeof region === 'object' → it is a full region object { id, name, terrain, weather, isDeathZone, ... }
```

For bare string IDs, you may look up the full region in `view.visibleRegions` when the
region is visible; otherwise treat fields other than the ID as unknown.

```ts
function resolveRegion(entry, view) {
  if (typeof entry === 'object') return entry;                    // full object
  return view.visibleRegions.find((r) => r.id === entry) ?? null; // null → out-of-vision
}
```
